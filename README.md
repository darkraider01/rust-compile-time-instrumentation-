...
> **Project Positioning:** Researching zero-code compile-time OpenTelemetry instrumentation for Rust applications and Cargo dependencies.
...
- [Appendix E — Experiment matrix](docs/appendix-e-experiment-matrix.md) *(the completed validation experiments, source verifications, and prioritized remaining experimental questions)*
...
| Dependency coverage works for **async** functions, not just synchronous ones | **Medium — mechanism demonstrated** | A hand-written `core`-only `OtelFuture` wrapper reproducing `FutureExt::with_context` over the C ABI was verified under forced single-thread interleaving with positive control ([Appendix E](docs/appendix-e-experiment-matrix.md) E-8, closing FE-2). Invariants S4/S5/S6 held. Remaining uncertainty is end-to-end integration into the automated byte-range splicer pipeline (FE-13) |
| The tool works on macOS, Windows, and Linux — the platforms eBPF cannot reach | **Medium — demonstrated on Windows and Linux** | Sync trampolines and release-profile builds (`lto=true`, `codegen-units=1`, `panic=abort`) verified on Windows/MSVC (E-7) and Linux/ELF on WSL2 Ubuntu (E-9, partially closing FE-7). macOS (Mach-O) remains untested and is the sole platform gap ([R24](docs/13-technical-risks.md)) |
