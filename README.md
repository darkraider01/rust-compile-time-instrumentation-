...
- [~] **P2.3 - First-Party Lint-Apply Driver (`cargo instrument-rust`)** - BOUNDED VERTICAL SLICE COMPLETE
  `cargo instrument-rust --apply` clean-tree-gates a first-party-only `cargo fix` run, puts the isolated nightly `rustc_driver` HIR frontend in `RUSTC`, and leaves Cargo's `RUSTC_WRAPPER` diagnostics proxy intact. It emits genuine `MachineApplicable` edits for ordinary free functions, inherent methods, and `async fn` bodies when `opentelemetry` is present. The persistent marker `/* __cargo_instrument_rust:p23 */` makes the command idempotent. The process fixture proves edit, dependency-source immutability, stable rebuild, dirty-tree refusal, and a committed no-op second run. The stable workspace does not depend on `rustc_private`; the driver requires nightly plus `rustc-dev`. Broader eligibility, preview UX, and parity remain P2.3 follow-up work.
