...
- [ ] **P2.3 - First-Party Lint-Apply Driver (`cargo instrument-rust`)** - IN PROGRESS ◀── current
  Will implement the default first-party workflow via `rustc_private` (`rustc_lint` and `rustc_errors`) per [ADR-012](docs/phase2/decision-records.md#adr-012---hybrid-first-partydependency-instrumentation-architecture) and the frozen integration contract in [ADR-013](docs/phase2/decision-records.md#adr-013---p23p24-architecture-freeze-and-cargo-fix-integration). The planned CLI provides visible compiler diagnostics (`--show`) and clean-tree-gated source modifications (`--apply`), eliminating recurring build overhead after edits are committed. The temporary feasibility experiment proved HIR body discovery, `MachineApplicable` diagnostics, Cargo-fix application, and a stable rebuild; the production driver and its idempotence/safety gates remain to be implemented.
