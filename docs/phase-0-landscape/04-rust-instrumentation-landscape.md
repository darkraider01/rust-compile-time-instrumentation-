...
**[Inference — directly relevant]** This gives an auto-instrumentation tool a free, idiomatic "off switch": generate spans at `TRACE`/`DEBUG` level and users who do not want them compile them out entirely, with no runtime cost and no need for our tool to have a "disable" mode. That is a strong argument for generating `tracing` calls rather than raw OTel API calls.
...
**[Inference]** `#[instrument]` is the correct *code generation target* for a source-level tool. It already solved the async problem correctly, it handles `Result`, and it is what Rust developers already read and understand. Generating `#[instrument]` attributes means our output is idiomatic, reviewable, and debuggable — a large practical advantage over generating raw span-construction code.
